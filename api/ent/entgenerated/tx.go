...
func (tx *Tx) init() {
	tx.EmailCredential = NewEmailCredentialClient(tx.config)
	tx.LoginSession = NewLoginSessionClient(tx.config)
	tx.SuperuserProfile = NewSuperuserProfileClient(tx.config)
	tx.User = NewUserClient(tx.config)
	tx.UserPublicProfile = NewUserPublicProfileClient(tx.config)
}
...
// applies a query, for example: EmailCredential.QueryXXX(), the query will be executed
