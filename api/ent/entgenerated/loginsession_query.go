...
type LoginSessionQuery struct {
	...
	loadTotal  []func(context.Context, []*LoginSession) error
...
func (lsq *LoginSessionQuery) QueryOwner() *UserQuery {
...
func (lsq *LoginSessionQuery) WithOwner(opts ...func(*UserQuery)) *LoginSessionQuery {
	query := (&UserClient{config: lsq.config}).Query()
...
//		GroupBy(loginsession.FieldOwnerID).
...
//
//	client.LoginSession.Query().
//		Select(loginsession.FieldOwnerID).
//		Scan(ctx, &v)
...
func (lsq *LoginSessionQuery) prepareQuery(ctx context.Context) error {
	...
	if loginsession.Policy == nil {
		return errors.New("entgenerated: uninitialized loginsession.Policy (forgotten import entgenerated/runtime?)")
	}
	if err := loginsession.Policy.EvalQuery(ctx, lsq); err != nil {
		return err
	}
	return nil
}
...
func (lsq *LoginSessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*LoginSession, error) {
	...
	if query := lsq.withOwner; query != nil {
		if err := lsq.loadOwner(ctx, query, nodes, nil,
			func(n *LoginSession, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
...
func (lsq *LoginSessionQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*LoginSession, init func(*LoginSession), assign func(*LoginSession, *User)) error {
...
func (lsq *LoginSessionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := lsq.ctx.Fields; len(fields) > 0 {
		...
		if lsq.withOwner != nil {
			_spec.Node.AddColumnOnce(loginsession.FieldOwnerID)
		}
	}
